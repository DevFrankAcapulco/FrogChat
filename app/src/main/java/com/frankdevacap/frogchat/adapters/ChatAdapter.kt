...
import com.frankdevacap.frogchat.utils.CircleTransform
import com.frankdevacap.mylibrary.interfaces.inflate
    ...
    class ViewHolderR(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(message: Message) = with(itemView) {
            ...
            if(message.profileImageURL.isEmpty()) {
                Picasso.get().load(R.drawable.ic_person).resize(80, 80)
                    .centerCrop().transform(CircleTransform()).into(imageViewProfileRight)
            } else {
                Picasso.get().load(message.profileImageURL).resize(80, 80)
                    .centerCrop().transform(CircleTransform()).into(imageViewProfileRight)
            }
        }
    }
